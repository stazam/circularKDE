...
test_that("bw.ccv removes NA values and returns result", {
  ...
  result <- bw.ccv(x)
  expect_type(result, "double")
  expect_cli_warning(bw.ccv(x), 1, "! `x` contains missing values, which will be removed")
})

test_that("bw.ccv handles non-numeric lower", {
  x <- seq(0, 2 * pi, length.out = 5)
  expect_cli_warning(result <- bw.ccv(x, lower = "zero"), 1, "! Argument `lower` must be numeric. Default value 0 for lower boundary was used.")
  expect_type(result, "double")
})

test_that("bw.ccv handles non-numeric upper", {
  x <- seq(0, 2 * pi, length.out = 5)
  expect_cli_warning(result <- bw.ccv(x, upper = "sixty"), 1, "! Argument `upper` must be numeric. Default value 60 for upper boundary was used.")
  expect_type(result, "double")
})

test_that("bw.ccv warns and resets invalid boundary values", {
  x <- seq(0, 2 * pi, length.out = 5)
  expect_cli_warning(result <- bw.ccv(x, lower = -5, upper = 5), 1, "! The boundaries must be positive numbers and 'lower' must be smaller that 'upper'. Default boundaries lower=0, upper=60 were used.")
  expect_type(result, "double")

  expect_cli_warning(result <- bw.ccv(x, lower = 10, upper = 5), 1, "! The boundaries must be positive numbers and 'lower' must be smaller that 'upper'. Default boundaries lower=0, upper=60 were used.")
  expect_type(result, "double")
})

...
test_that("bw.lscvg handles non-numeric g", {
  ...
  expect_cli_warning(result <- bw.lscvg(x, g = "wrong"), 1, "! Argument `g` must be numeric. Default value 4 for coefficient was used.")
  expect_type(result, "double")
})

test_that("bw.lscvg handles non-numeric lower", {
  x <- seq(0, 2 * pi, length.out = 5)
  expect_cli_warning(result <- bw.lscvg(x, lower = "zero"), 1, "! Argument `lower` must be numeric. Default value 0 for lower boundary was used.")
  expect_type(result, "double")
})

test_that("bw.lscvg handles non-numeric upper", {
  x <- seq(0, 2 * pi, length.out = 5)
  expect_cli_warning(result <- bw.lscvg(x, upper = "sixty"), 1, "! Argument `upper` must be numeric. Default value 60 for upper boundary was used.")
  expect_type(result, "double")
})

test_that("bw.lscvg warns and resets invalid boundary values", {
  x <- seq(0, 2 * pi, length.out = 5)
  expect_cli_warning(result <- bw.lscvg(x, lower = -5, upper = 5), 1, "! The boundaries must be positive numbers and 'lower' must be smaller that 'upper'. Default boundaries lower=0, upper=60 were used.")
  expect_type(result, "double")

  expect_cli_warning(result <- bw.lscvg(x, lower = 10, upper = 5), 1, "! The boundaries must be positive numbers and 'lower' must be smaller that 'upper'. Default boundaries lower=0, upper=60 were used.")
  expect_type(result, "double")
})

test_that("bw.lscvg warns when minimum is at edge of the range", {
  x <- rep(0, 10)
  expect_cli_warning(bw.lscvg(x), 1, "! Minimum/maximum occurred at one end of the range.")
})
